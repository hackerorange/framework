...
public class SimpleLunarCalendar {
	...
	private void init(long timeInMillis) {
		if (timeInMillis > minTimeInMillis && timeInMillis < maxTimeInMillis) {
			...
			this.maxDayInMonth = daysInLunarMonth != 0 ? daysInLunarMonth
					: SimpleLunarCalendar.getLunarMonthDays(this.lunarYear, lunarMonth);
	...
	private static int getLunarLeapDays(int lunarYear) {
		...
		// 若该年没有闰月,返回0
		return SimpleLunarCalendar.getLunarLeapMonth(lunarYear) > 0
				? ((SimpleLunarCalendar.lunarInfo[lunarYear - 1899] & 0xf) == 0xf ? 30 : 29) : 0;
	}
